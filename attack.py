...
if __name__ == '__main__':
    ...
    init = Init(seed=9970, log_root_dir=root_dir,
                backup_filename=__file__, tensorboard=True, comment=f'attack AT and T face')
    ...
    def Attack(mynet, target_label):
        ...
        writer.add_image(f'original input image {target_label}',
                         aim_flatten.detach().reshape(1, h, w), target_label)
        writer.add_text(f'original input image predict label {target_label}',
        ...
        after_softmax = F.softmax(out, dim=-1)
        predict = torch.argmax(after_softmax)
        writer.add_image(f'inverted image {target_label}',
                         aim_flatten.detach().reshape(1, h, w), target_label)
        writer.add_text(f'inverted image predict label {target_label}',
                        f'predict label: {predict.item()}')
        save_image(aim_flatten.detach().reshape(h, w),
                   f'{log_dir}/inverted_{target_label}.png')
    ...
    mynet.load_state_dict(torch.load(
        open(target_pkl, 'rb'), map_location=output_device))
